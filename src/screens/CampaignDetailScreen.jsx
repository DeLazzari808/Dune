...
import { ArrowLeft, MapPin, CalendarDays, CheckCircle, Bookmark } from 'lucide-react';
import { colors } from '../config';

const CampaignDetailScreen = ({ campaign, setCurrentPage }) => {
  if (!campaign) {
    return (
      <div className="p-4 text-center" style={{backgroundColor: colors.pageBg, color: colors.textMain, minHeight: 'calc(100vh - 4rem)'}}>
        ...
        <button onClick={() => setCurrentPage('Feed')} className="mt-4 px-4 py-2 rounded-lg border" style={{backgroundColor: colors.buttonBg, color: colors.buttonText, borderColor: colors.buttonBorder}}>Voltar para Agenda</button>
      </div>
    );
  }
